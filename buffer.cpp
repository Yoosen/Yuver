...
size_t Buffer::readBytes() const { return readPos_; }

const char* Buffer::curReadPtr() const { return BeginPtr_() + readPos_; }

const char* Buffer::curWritePtrConst() const { return BeginPtr_() + writePos_; }

char* Buffer::curWritePtr() { return BeginPtr_() + writePos_; }
...
ssize_t Buffer::readFd(int fd, int* Errno) {
    ...
    struct iovec iov[2];
    const size_t writable = writeableBytes();  //缓存区中可以读取的字节数

    iov[0].iov_base = BeginPtr_() + writePos_;    // 返回指向缓冲区初始位置的指针 + 用于指示写指针
